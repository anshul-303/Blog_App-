import { ThumbsUp, MessageCircle, User, Calendar } from "lucide-react";

export default function AuthorPublishedCard({
  blogId,
  title,
  summary,
  author,
  likesCount,
  commentsCount,
  status,
  createdAt,
}) {
  return (
    <div className="bg-zinc-800 md:w-[47vw] md:min-h-[26vh] min-h-[18vh] w-[93vw] mr-4 md:m-0 flex flex-col justify-start items-start p-6 border border-zinc-700 rounded-lg border-[2px] gap-2">
      <p
        className="relative inline-block text-xs font-semibold px-3 py-1 rounded-full 
                bg-gradient-to-r from-emerald-400 to-emerald-500 text-emerald-900
                shadow-sm shadow-emerald-400/40
                hover:shadow-emerald-400/70 hover:brightness-110
                transition-all duration-200"
      >
        {status}
      </p>
      <p className=" text-2xl md:text-3xl font-semibold text-zinc-200 ">
        {title}
      </p>
      <p className=" md:text-[1em] text-sm text-zinc-500 flex gap-1  justify-center ">
        {summary}
      </p>
      <div className="flex gap-4">
        <p className="inline text-md text-zinc-600 flex">
          <User className="w-4 h-4 inline" />{" "}
          <span className="font-semibold text-sm text-zinc-400">{author}</span>
        </p>{" "}
        <p className="inline text-md text-zinc-600 flex">
          <Calendar className="w-4 h-4 inline" />
          <span className="font-semibold text-sm text-zinc-400">
            &nbsp;{createdAt}
          </span>
        </p>
        <p className="inline text-md text-zinc-600 flex">
          <ThumbsUp className="w-4 h-4 inline" />
          <span className="font-semibold text-sm text-zinc-400">
            &nbsp;{likesCount}
          </span>
        </p>
        <p className="inline text-md text-zinc-600 flex">
          <MessageCircle className="w-4 h-4 inline" />
          <span className="font-semibold text-sm text-zinc-400">
            &nbsp;{commentsCount}
          </span>
        </p>
      </div>
    </div>
  );
}
